...
func main() {
	...
	// (which would otherwise skip the defer), or the SIGTERM/SIGINT signal handler
	// installed below. telemetry.Shutdown caps the flush at its own 3s
	// ShutdownTimeout; we give it a 5s outer budget, well within the pod's default
	// 30s terminationGracePeriod (ISI-1483).
	...
	// instead of fragmenting into N disconnected single-run traces (ISI-1482/1485).
