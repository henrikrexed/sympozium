...
// JetStream server and proves the drain() generation-check fix:
//
...
//     made a consumerLost-only recovery spin forever. The generation check
//     re-establishes the consumer anyway.
//
// Both subscriber flavors are covered: the ephemeral Subscribe and the durable
// queue-group SubscribeGroup the live ChannelRouter actually uses (ISI-1430).
// Both share drain(), so the fix is identical; the live symptom was on the
// durable path.
//
// Before the fix this test hangs at the "consumer re-established" wait: the
// stream re-created and the connected gauge returned to 1, but the subscriber
// never re-created its consumer (Active Consumers stayed 0) and the probe sat
// UNDELIVERED until a process restart.
func TestSubscriberReEstablishesConsumerAfterFullStreamRecreate(t *testing.T) {
	...
	cases := []struct {
		name      string
		subscribe func(ctx context.Context, bus *NATSEventBus, topic string) (<-chan *Event, error)
	}{
		{
			name: "ephemeral/Subscribe",
			subscribe: func(ctx context.Context, bus *NATSEventBus, topic string) (<-chan *Event, error) {
				return bus.Subscribe(ctx, topic)
			},
		},
		{
			name: "durable/SubscribeGroup",
			subscribe: func(ctx context.Context, bus *NATSEventBus, topic string) (<-chan *Event, error) {
				return bus.SubscribeGroup(ctx, topic, "channel-router")
			},
		},
	}
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			ch, err := tc.subscribe(ctx, bus, topic)
			if err != nil {
			...
			// The healer re-creates the stream on reconnect; its new Created
			// timestamp advances the generation. This is the "gauge flips back to
			// 1" milestone — necessary but, per ISI-1470, NOT sufficient alone.
			...
			// routed to the subscriber (the production drill's probe sat
			// UNDELIVERED). Published only after the consumer exists, since
			// DeliverNew would skip a message published before re-establishment.
