...
func TestTriggerSequentialSuccessors_PropagatesReplyAnnotations(t *testing.T) {
	...
	// Trace context must still be carried alongside the reply annotations.
	if succ.Annotations["otel.dev/traceparent"] != "00-aaa-bbb-01" {
		t.Errorf("traceparent not propagated: %q", succ.Annotations["otel.dev/traceparent"])
	}
}
